...
network.fc_1.weight = torch.nn.Parameter(torch.eye(2, requires_grad = False, device = device))
...
if finetune == True:
    network.load_state_dict(torch.load('models/v4.6/classifier'))
...
for epoch in range(epochs):
    ...
    if valid_loss < min(loss_history):
        print('A better model has been obtained. Saving this model to models/v4.6/classifier')
        torch.save(network.state_dict(), 'models/v4.6/classifier')
