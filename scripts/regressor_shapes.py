...
class Network(nn.Module):
    ...
    def forward(self, x):
        ...
        conv_3 = F.relu(self.conv2(conv_2))
...
print('Load data')
train_X, train_Y = loadData('data/shapes/train.h5')
valid_X, valid_Y = loadData('data/shapes/valid.h5')
...
if 'TRAIN' in MODE:
    if finetune == True:
        network.load_state_dict(torch.load('models/shapes/regressor'))
    ...
    for epoch in range(epochs):
        ...
        if valid_loss < min(loss_history):
            print('A better model has been obtained. Saving this model to models/shapes/regressor')
            torch.save(network.state_dict(), 'models/shapes/regressor')
...
if 'TEST' in MODE:
    ...
    # Testing
    network.load_state_dict(torch.load('models/shapes/regressor', map_location = lambda storage, loc : storage))
    ...
    with open('models/shapes/results_regressor.txt', 'w') as f:
